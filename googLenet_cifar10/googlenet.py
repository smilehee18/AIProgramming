...
class GoogLeNet(nn.Module):
    ...
    def eager_outputs(self, x, aux2, aux1):
        ...
        if self.training and self.aux_logits:
            return _GoogLeNetOutputs(x, aux2, aux1) #train일 때는 tuple 반환
        else:
            return x #test모드일때는 출력값만 반환
...
class InceptionAux(nn.Module):
    ...
    def forward(self, x):
        ...
        # N x 1024
        x = F.dropout(x, 0.99, training=self.training)
        # N x 1024
        x = self.fc2(x)
        # N x 1000 (num_classes)
        return x
...
class BasicConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, **kwargs):
        ...
        self.bn = nn.BatchNorm2d(out_channels, eps=0.002)
